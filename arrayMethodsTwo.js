...
const pewpew = numbers.map( rat => rat/2)
console.log(pewpew)
...
const citiesWithI = cities.filter( rat => rat.charAt(rat.length-1) === "i")
...
/* 
    ...
    If the color is red, print "The fruit with index [THE ELEMENT'S INDEX] is an apple." If the color is orange, print "The fruit with index [THE ELEMENT INDEX] is an orange." 
...
fruits.forEach((rat) => {
  if (rat.color === "red") {
    return console.log(
      `The fruit with index ${fruits.indexOf(rat)} is an apple.`
    );
  } else if (rat.color === "orange") {
    return console.log(
      `The fruit with index ${fruits.indexOf(rat)} is an orange.`
    );
  } else {
    return console.log(
      `The fruit with index ${fruits.indexOf(rat)} is neither apple or orange.`
    );
  }
});
...
const pewpewpew = foods.filter( rat => rat.tags.includes('rice')).reduce( (sum,rat) => sum + rat.price,0)
