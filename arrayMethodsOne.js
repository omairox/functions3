...
// CODE HERE
const evenNumbers = mixedNumbers.filter(rat => rat % 2 === 0)
...
// // CODE HERE
const postTaxPrices = prices.map(rat => rat * 1.07);
...
// CODE HERE
const totalPopulation = populations.reduce( (sum,rat) => {
  return sum + rat;
})
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter( rat => {
  return rat.CP > 200
})
...
const eachTotal = orders.map( rat => {
  return rat.price * (rat.tax + 1)
})
...
const bobsTotal = purchases.filter(rat => rat.owner === 'Bob').reduce( (sum, rat) => sum + rat.price, 0)
